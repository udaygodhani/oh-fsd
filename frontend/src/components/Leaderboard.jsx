...
const Leaderboard = () => {
    ...
    const [activeModalTab, setActiveModalTab] = useState("coins"); // Missing state
    const [filteredUsers, setFilteredUsers] = useState([]); // Assuming needed, but kept minimal
    ...
    // Added missing filteredCoins (duplicated logic for modal)
    const filteredCoins = filteredAndSortedCoins;
                    ...
                    {filteredAndSortedCoins.map((coin, index) => (
                        ...
                        >
                            ...
                            <div className="text-right">
                                ...
                                <div className={`text-sm font-medium flex items-center justify-end gap-1 ${coin.change >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {coin.change >= 0 ? <FiTrendingUp /> : <FiTrendingDown />}
                                    {coin.change}%
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 py-4 bg-[#1B1336]/40 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-purple-500/10">
                            ...
                            <div className="flex bg-[#0B081E] p-1.5 rounded-2xl border border-purple-500/15 w-full md:w-auto">
                                <button
                                    ...
                                    className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${activeModalTab === "coins"
                                ...
                                <button
                                    ...
                                    className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${activeModalTab === "users"
                                        ? "bg-purple-600 text-white shadow-md shadow-purple-500/20"
                                        : "text-gray-400 hover:text-white"
                                        }`}
                                >
                                            ...
                                            {filteredCoins.length > 0 ? (
                                                filteredCoins.map((coin) => (
                                                    ...
                                                    >
                                                        ...
                                                        <td className={`py-4 text-right font-semibold font-mono ${String(coin.change).startsWith("+") ? "text-green-400" : "text-red-400"
                                                            }`}>
